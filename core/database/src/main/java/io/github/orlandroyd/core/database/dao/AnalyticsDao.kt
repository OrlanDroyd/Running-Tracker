package io.github.orlandroyd.core.database.dao
...
interface AnalyticsDao {
    ...
    @Query("SELECT AVE(distanceMeters) FROM runentity")
    suspend fun getAvgDistancePerRun(): Double

    @Query("SELECT AVE(durationMillis / 60000.0 / (distanceMeters / 1000.0)) FROM runentity")
